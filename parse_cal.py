...
# function to parse a single event and return event dictionary
def parse_event(event, allow_unaccepted=False, always_allow_senders=[], site_owner_email=None):
    ...
    if event_dict['description'] is not None:
        ...
        try:
            ...
            w = 'Could not parse description for event with UID ' + str(event_dict['uid']) + '. Error: ' + str(e)
            warnings.warn(w)
            event_dict['description'] = None
...
def parse_cal(cal, outpaths = [], allow_unaccepted = False, always_allow_senders = [], site_owner_email = None, include_recurring_events_days = 14):
    ...
    for event in raw_events:
        try:
            event_dict = parse_event(event, allow_unaccepted, always_allow_senders, site_owner_email)
        except Exception as e:
            w = 'Could not parse event. Error: ' + str(e)
            warnings.warn(w)
            event_dict = None
